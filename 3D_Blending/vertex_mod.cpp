...
#include <math.h>
using namespace std;
vertex_mod::vertex_mod()
{
    noOfVertices = 0;
    noOfFaces = 0;
}
//----------------------------------------------------------------------
bool vertex_mod::read3DModel(const char *filename)
{
	bool succ = false;
...
//----------------------------------------------------------------------
bool vertex_mod::readPLY(const char *filename)
{
...
//------------------------------------------------------------------------
bool vertex_mod::read3DSkeleton(const char *filename)
{
	bool succ = false;
...
//-----------------------------------------------------------------------
bool vertex_mod::readSKEL(const char *filename)
{
	int i;
...
//------------------------------------------------------------------------
bool vertex_mod::readSegment(const char *filename)
{
	bool succ = false;
...
bool vertex_mod::readSEG(const char *filename)
{
...
void vertex_mod::setSegmentColor(int segID)
{
...
//------------------------------------------------------------------------
void vertex_mod::DisplayVertices()
{
    //int i;
...
//--------------------------------------------------------------------
void vertex_mod::DisplayFaces(float transparent)
{
    float x1,y1,z1;
    float x2,y2,z2;
    float x3,y3,z3;
...
//-------------------------------------------------------------------------
double vertex_mod::getX(int index)
{
    return sr_verts[index].v[0];
}
//-------------------------------------------------------------------------
double vertex_mod::getY(int index)
{
    return sr_verts[index].v[1];
}
//-------------------------------------------------------------------------
double vertex_mod::getZ(int index)
{
    return sr_verts[index].v[2];
}
//--------------------------------------------------------------------------
vector<float> vertex_mod::calculateNormal( float *coord1, float *coord2, float *coord3 )
{
...
bool vertex_mod::readSkelSegment(const char *filename)
{
...
int vertex_mod::checkEdgeSegment(int skelID1, int skelID2)
{
...
bool vertex_mod::readBlendedSegment(const char* filename)
{
    //*//
    string line;
    ifstream readIn(filename, ios::in);
    ...
    if(readIn)
    {
        while(!readIn.eof())
        {
            ...
            p.c[ENUM_Y] = y;
            p.c[ENUM_Z] = z;
            vectorOfBlendedSegment.push_back(p);
...
void vertex_mod::deleteOldSegments()
{
